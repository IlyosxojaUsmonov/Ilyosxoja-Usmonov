...
export const Route = createFileRoute("/resume")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Digital resume of Ilyosxoja Usmonov: junior beginner with React, Tailwind CSS, and freelance experience since 2024." },
    { property: "og:title", content: "Resume — Ilyosxoja Usmonov" },
    { property: "og:description", content: "Frontend developer resume, education, skills, freelance experience." },
...
const skills: { g: Record<Lang, string>; list: string[] }[] = [
  ...
  { g: { uz: "Frameworklar / Kutubxonalar", en: "Frameworks / Libraries" }, list: ["React", "Tailwind CSS", "Framer Motion"] },
  { g: { uz: "Asboblar", en: "Tools" }, list: ["Figma", "Git", "Vite", "REST APIs"] },
];

const experience: { title: Record<Lang, string>; org: Record<Lang, string>; date: string; bullets: Record<Lang, string[]> }[] = [
  {
      ...
      <div className="mx-auto max-w-5xl px-5 sm:px-8 grid gap-6 pb-16">
        ...
        <section className="rounded-3xl bg-white border border-border p-8">
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="text-primary font-semibold mt-1">Junior beginner</div>
            ...
            <div className="md:col-span-2">
              <div className="text-xs uppercase tracking-widest text-primary font-semibold">{t("resume.summary")}</div>
        ...
        <section className="rounded-3xl bg-white border border-border p-8">
          ...
          <div className="mt-4 flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <div className="text-muted-foreground">Junior beginner track</div>
            </div>
            ...
            {skills.map((s, i) => (
              <div key={i}>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {s.list.map((it) => <span key={it} className="text-xs px-2.5 py-1 rounded-full bg-primary/8 text-primary border border-primary/15">{it}</span>)}
                </div>
              </div>
            ))}
          </div>
          ...
          {experience.map((e, i) => (
            <div key={i} className="mt-4">
              ...
              <ul className="mt-3 space-y-1.5">
                {e.bullets[lang].map((b) => <li key={b} className="flex gap-2 text-sm"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-primary shrink-0" />{b}</li>)}
              </ul>
            </div>
          ))}
        </section>
        ...
        <section className="rounded-3xl bg-white border border-border p-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <div className="px-4 py-2 rounded-full bg-primary/8 text-primary border border-primary/20 font-medium">English — Professional</div>
          </div>
        </section>
