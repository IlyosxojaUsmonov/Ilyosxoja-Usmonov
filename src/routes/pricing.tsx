...
const tiers: Tier[] = [
  {
    name: "Starter",
    positioning: { uz: "Bir sahifali marketing sayti uchun ideal.", en: "Best for a single-page marketing site." },
    price: "$—",
    features: {
  ...
  {
    name: "Growth",
    positioning: { uz: "Kompaniya yoki shaxsiy brend uchun ko'p sahifali sayt.", en: "Multi-page company or personal brand site." },
    price: "$—",
    popular: true,
    features: {
  ...
  {
    name: "Premium",
    positioning: { uz: "Full-stack ilova va dashboard tizimlari uchun.", en: "Full-stack apps with backend or dashboard needs." },
    price: "$—",
    features: {
...
const compareRows: { label: Record<Lang, string>; values: (string | boolean)[] }[] = [
  { label: { uz: "Sahifalar soni", en: "Pages" }, values: ["1", "5–7", "∞"] },
  { label: { uz: "Responsive", en: "Responsive" }, values: [true, true, true] },
  { label: { uz: "Animatsiyalar", en: "Animations" }, values: [false, true, true] },
  { label: { uz: "Backend integratsiya", en: "Backend integration" }, values: [false, false, true] },
  { label: { uz: "Qo'llab-quvvatlash", en: "Support period" }, values: ["14d", "30d", "60d"] },
  { label: { uz: "Qayta ko'rib chiqish", en: "Revisions" }, values: ["2", "3", "∞"] },
];
...
function Pricing() {
  ...
  return (
    <>
      <PageHeader kicker={lang === "uz" ? "Paketlar" : "Packages"} title={t("pricing.title")} sub={t("pricing.sub")} />

      <section className="mx-auto max-w-7xl px-5 sm:px-8 grid md:grid-cols-3 gap-5">
        {tiers.map((tier) => (
          <div key={tier.name} className={`relative rounded-3xl p-8 border transition-all ${tier.popular ? "bg-gradient-to-br from-primary to-primary-glow text-white border-primary shadow-2xl shadow-primary/30 md:-translate-y-4" : "bg-white border-border"}`}>
            {tier.popular && (
            ...
            <div className="mt-6 flex items-baseline gap-1">
              ...
              <span className={tier.popular ? "text-white/70" : "text-muted-foreground"}>/{lang === "uz" ? "loyiha" : "project"}</span>
            </div>
